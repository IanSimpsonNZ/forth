...
create no-words max-line allot

variable buff-len
variable no-words-len
variable filter-len
...
: check-nums            ( start-pos -- n f )
    ...
    2dup one 3 same? if [char] 1 -1
    else 2dup two 3 same? if [char] 2 -1
    else 2dup three 5 same? if [char] 3 -1
    else 2dup four 4 same? if [char] 4 -1
    else 2dup five 4 same? if [char] 5 -1
    else 2dup six 3 same? if [char] 6 -1
    else 2dup seven 5 same? if [char] 7 -1
    else 2dup eight 5 same? if [char] 8 -1
    else 2dup nine 4 same? if [char] 9 -1
    else 0 0
...
: filter-words  ( -- )
    ...
    buff-len @ 0 do
        i check-nums if
            ( ascii for char is on stack )
        else
            drop
            line-buffer i + c@
        then
        no-words no-words-len @ + c! 
        1 no-words-len +!
    loop
;
